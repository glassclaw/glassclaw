...
// Load the pro catalog for component validation.
...
/**
 * Validate a single component against the catalog schema.
 * Returns an array of error strings (empty if valid).
 */
function _validateComponent(comp, lineNum) {
    const errors = [];
    const compName = comp.component || comp.type;
    if (!compName) return errors; // caught by the "missing component" check

    const schema = CATALOG_COMPONENTS[compName];
    if (!schema) {
    ...
    const knownProps = new Set([...Object.keys(schemaProps), 'id', 'component', 'type']);
    for (const key of Object.keys(comp)) {
        if (!knownProps.has(key)) {
            errors.push(`Component "${comp.id || '?'}" in line ${lineNum}: unknown property "${key}" on ${compName}. Valid properties: ${Object.keys(schemaProps).join(', ')}.`);
        }
    }

    // Check for missing required properties.
    for (const req of requiredProps) {
        if (comp[req] === undefined) {
            errors.push(`Component "${comp.id || '?'}" in line ${lineNum}: missing required property "${req}" on ${compName}.`);
        }
    }

    // Check property types.
    for (const [key, val] of Object.entries(comp)) {
        if (key === 'id' || key === 'component' || key === 'type') continue;
        const propSchema = schemaProps[key];
        if (!propSchema) continue; // already caught as unknown

        // Check for {literalString: ...} wrappers (common mistake).
        ...
        if (!expectedType) continue; // oneOf or complex type, skip

        // Skip type check for data binding objects {path: "/..."}
        if (val && typeof val === 'object' && !Array.isArray(val) && 'path' in val) continue;

        if (expectedType === 'string' && typeof val !== 'string') {
            // Allow numbers for string fields (they get converted)
            if (typeof val !== 'number') {
                errors.push(`Component "${comp.id || '?'}" property "${key}": expected string, got ${typeof val}.`);
            }
        } else if (expectedType === 'number' && typeof val !== 'number') {
            if (typeof val === 'string' && !isNaN(Number(val))) { /* allow numeric strings */ }
            else {
                errors.push(`Component "${comp.id || '?'}" property "${key}": expected number, got ${typeof val}.`);
            }
        } else if (expectedType === 'array' && !Array.isArray(val)) {
            if (val && typeof val === 'object' && 'explicitList' in val) { /* adapter format, ok */ }
            else {
                errors.push(`Component "${comp.id || '?'}" property "${key}": expected array, got ${typeof val}.`);
            }
        } else if (expectedType === 'boolean' && typeof val !== 'boolean') {
            // Allow string booleans
            if (val !== 'true' && val !== 'false') {
                errors.push(`Component "${comp.id || '?'}" property "${key}": expected boolean, got ${typeof val}.`);
            }
        }
    }
...
async function get_account() {
    requireConfig();
    return glassclaw('GET', '/api/account');
}
...
function _deliverToAgent(surfaceId, chatId, formData, log, iteration) {
    ...
    try {
        log(`POLL #${iteration} — looking up openclaw session for chatId=${chatId}.`);
        const sessionsJson = execSync('openclaw sessions --json', { timeout: 10000 }).toString();
        ...
        execSync(
            `openclaw agent --message ${JSON.stringify(messageText)} --session-id ${sessionId} --deliver`,
...
if (require.main === module) {
    ...
    if (!command) {
        ...
        console.error('Commands: config, setup, create_card, update_card, delete_card, poll_response, get_account');
        process.exit(1);
    }
    ...
    const commands = { setup, create_card, update_card, delete_card, poll_response, get_account };
